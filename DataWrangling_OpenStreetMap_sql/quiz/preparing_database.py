...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
        ...
        for attributes, values in element.attrib.items():
            if attributes in node_attr_fields:
                # print(attributes,values)
                node_attribs[attributes] = values
        for child in element.iter():
            if child.tag == 'tag':
                ...
                if problem_chars.match(child.attrib['k']) is not None:
                    ...
                    var = tag_attributes(element, default_tag_type, child)
                    tags.append(var)
                    # print(tags)
        return {'node': node_attribs, 'node_tags': tags}
    elif element.tag == 'way':
        for attributes, values in element.attrib.items():
            if attributes in way_attr_fields:
                # print(attributes,values)
                way_attribs[attributes] = values
        pos = 0
        for child in element.iter():
            if child.tag == 'tag':
                if problem_chars.match(child.attrib['k']) is not None:
            ...
            if child.tag == 'nd':
                ...
                new_node['node_id'] = child.attrib['ref']
